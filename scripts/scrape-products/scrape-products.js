...
async function scrapeProductsFromPage(url) {
  ...
  try {
    ...
    if (url.includes("woolworths.com.au")) {
      productElements = $(".product-grid-v2--tile");
      console.log(`Found ${productElements.length} products with Woolworths selector: shared-product-tile`);
    } else if (url.includes("coles.com.au")) {
      productElements = $('#coles-targeting-product-tiles > [data-testid="product-tile"]');
      console.log(`Found ${productElements.length} products with Coles selector: #coles-targeting-product-tiles > [data-testid="product-tile"]`);
    }
